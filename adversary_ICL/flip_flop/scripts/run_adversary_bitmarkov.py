"""CMA-ES adversary search over the BitMarkov axis (axis A1).

Search space (5 dims):
  x[0:3] -> softmax-3 -> (p_w, p_r, p_i)        # simplex
  x[3]   -> sigmoid    -> bit_p1   in [0, 1]
  x[4]   -> sigmoid    -> bit_stay in [0, 1]    # P(b_t == b_{t-1})

Usage:
    python -m flip_flop.scripts.run_adversary_bitmarkov \
        --config flip_flop/configs/adversary_bitmarkov.yaml
    python -m flip_flop.scripts.run_adversary_bitmarkov \
        --config flip_flop/configs/adversary_bitmarkov.yaml --test_run
"""
...
from flip_flop.adversary.distribution import BitMarkov
...
def _load_yaml_flat(path: str) -> dict:
    """Yaml loader that accepts both flat and nested-section configs."""
    with open(path) as f:
...
class BitMarkovEncoder:
    """Real-vector <-> BitMarkov distribution params (5 dims)."""

    def __init__(self, T: int):
        self.T = T
        self.n_dims = 5

    def decode(self, x: np.ndarray) -> BitMarkov:
        x = np.asarray(x)
        simplex = _softmax3(x[0:3])
        p_w, p_r, _p_i = simplex
        bit_p1 = float(_sigmoid(np.array([x[3]]))[0])
        bit_stay = float(_sigmoid(np.array([x[4]]))[0])
        return BitMarkov(T=self.T, p_w=float(p_w), p_r=float(p_r),
                         bit_p1=bit_p1, bit_stay=bit_stay)
...
def main():
    ...
    print(f"[adversary-bitmarkov] device={device} T={T} n={n} budget={budget} "
    ...
    if lstm_ckpt and lstm_cfg and os.path.exists(lstm_ckpt) and os.path.exists(lstm_cfg):
        ...
        print("[adversary-bitmarkov] no LSTM checkpoint; running without LSTM penalty")
